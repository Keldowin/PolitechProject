...
async def TerminSerchMethod(callback_query: types.CallbackQuery):
    await callback_query.answer(text="Правильный ответ! Следующий термин ➡",show_alert=True)
    
    await generate_test(callback_query)

@dp.callback_query_handler(lambda c: c.data == 'FalseAnswer')
async def TerminSerchMethod(callback_query: types.CallbackQuery):
    await callback_query.answer(text="Неправильный ответ! Посмотрите определение!",show_alert=True)
